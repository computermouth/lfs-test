...
sub get_replication_status {
	print "\n-------- Replication Status  -------------------------------------------\n";
	my $io_running = `mysql $mysqllogin -Bse "show slave status\\G"|grep -i slave_io_running|awk '{ print \$2}'`;
	my $sql_running = `mysql $mysqllogin -Bse "show slave status\\G"|grep -i slave_sql_running|awk '{ print \$2}'`;
	chop ($io_running);
	chop ($sql_running);
	...
	elsif ( $io_running eq 'Yes' && $sql_running eq 'No'){
		badprint "This replication is out of sync!\n";
	}
}
